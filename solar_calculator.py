...
def main():
    ...
    # We use session_state to remember if the Calculate button was clicked
    if 'calc_trigger' not in st.session_state:
        st.session_state.calc_trigger = False

    # === CALCULATE BUTTON ===
    # Always show the Calculate Solar Savings button
    if st.button("Calculate Solar Savings"):
        if bill is None or bill <= 0:
            # Invalid bill input: show warning
            st.warning("Please enter a valid monthly bill greater than 0 to perform calculation.")
            st.session_state.calc_trigger = False
        else:
            # Valid input: set calc_trigger and store the bill
            st.session_state.calc_trigger = True
            st.session_state.bill = bill

    # === SHOW RESULTS IF CALCULATION WAS TRIGGERED ===
    if st.session_state.get('calc_trigger', False):
        bill = st.session_state.bill  # Use stored bill value

        # Constants
        tariff = 0.63  # MYR per kWh
        sunlight_hours = 3.42  # Average sunlight hours per day
        panel_watt = 615  # Power per panel in watts
        system_life = 25  # Years

        # Price table for different panel packages
        price_table = {
            10: 21000,
            14: 26000,
            20: 34000,
        ...
        # LEFT COLUMN: Capacity, package, cost, monthly savings
        with col1:
            ...
            st.markdown(f"<div style='border:1px solid rgba(255,255,255,0.2); padding:10px; border-radius:8px; background:rgba(0,0,0,0.2); color:inherit'><strong>Estimated Monthly Savings:</strong><br> MYR {monthly_savings:.0f}</div>", unsafe_allow_html=True)

        # RIGHT COLUMN: yearly savings, payback, ROI, solar gen
        with col2:
            st.markdown(f"<div style='border:1px solid rgba(255,255,255,0.2); padding:10px; border-radius:8px; background:rgba(0,0,0,0.2); color:inherit'><strong>Estimated Yearly Savings:</strong><br> MYR {yearly_savings:.0f}</div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<div style='border:1px solid rgba(255,255,255,0.2); padding:10px; border-radius:8px; background:rgba(0,0,0,0.2); color:inherit'><strong>Bill Offset:</strong><br> {offset_percent:.1f}%</div>", unsafe_allow_html=True)
